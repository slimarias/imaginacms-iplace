...
@section('content-header')
    <h1>
        {{ trans('iplaces::cities.title.create city') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.iplaces.city.index') }}">{{ trans('iplaces::cities.title.cities') }}</a></li>
        <li class="active">{{ trans('iplaces::cities.title.create city') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.iplaces.city.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-9">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('iplaces::admin.cities.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
