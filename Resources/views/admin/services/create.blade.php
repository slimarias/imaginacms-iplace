...
@section('content-header')
    <h1>
        {{ trans('iplaces::services.title.create service') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.iplaces.service.index') }}">{{ trans('iplaces::services.title.services') }}</a></li>
        <li class="active">{{ trans('iplaces::services.title.create service') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.iplaces.service.store'], 'method' => 'post']) !!}
    <div class="row">
                        ...
                        <div class="box-body">
                            <select class="form-control" name="servtype" id="servtype">
                                @foreach ($servtypes as $index => $servtype)
                                    <option value="{{$index}}" {{ old('servtype',0) == $index ? 'selected' : '' }}>{{$servtype}}</option>
                                @endforeach
                            </select><br>
                        </div>
                    </div>
