...
@section('content-header')
    <h1>
        {{ trans('iplaces::spaces.title.create space') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.iplaces.space.index') }}">{{ trans('iplaces::spaces.title.spaces') }}</a></li>
        <li class="active">{{ trans('iplaces::spaces.title.create space') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.iplaces.space.store'], 'method' => 'post']) !!}
    <div class="row">
                            ...
                            <div class="tab-content">
                                ...
                                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                    ...
                                    <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                                        @include('iplaces::admin.spaces.partials.create-fields', ['lang' => $locale])
                                    </div>
                                @endforeach
